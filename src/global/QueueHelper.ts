...
export interface QueueHelperJSON {
...
export default class QueueHelper {
    ...
    public async hasMessages() {
        const peekMessage: (queue: string) => Promise<azs.QueueService.QueueMessageResult> =
            util.promisify(azs.QueueService.prototype.peekMessage).bind(this.service);
    ...
    constructor(obj: QueueHelperJSON) {
